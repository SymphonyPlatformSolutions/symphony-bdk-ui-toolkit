...
<Meta title="components/Financial/Charts" component={BaseChart} />
...
import { Box, useAutoFetch, BaseChart } from 'symphony-bdk-ui-toolkit';
...
export const AreaChartExample = withTheme(({ theme, height }) => {
  ...
  const xExtends = [xAccessor(last(results)), xAccessor(results[results.length])];
