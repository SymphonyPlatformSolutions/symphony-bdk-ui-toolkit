...
### J-Drizzle (alias: the Biggest Chungus) attempts what has never been done before

According to web dev lore, there exists an arcane and elusive datepicker which allows to the user not only to browse by day, but also by month -- and even year! Known to be an arduous (some might even say perilous) task, our Biggest Chungus ventures forth on the audacious journey to build such a datepicker in-house and unleash its power on the citizens of Symphony-UI-Toolkit (the people affectionately known as the SUITs).
